...
#
class TurtleOutOfBounds(Exception):
    pass
...
def new_position(x_coord, y_coord):
    ...
    assert fabs(y_coord) < 150, "y_coord is out of bounds"

    if fabs(x_coord) > 150:
        print(f"x_coord: {x_coord} is out of bounds")
        raise TurtleOutOfBounds
        
    
    if fabs(y_coord) > 150:
        print(f"y_coord: {y_coord} is out of bounds")
        return
